...
namespace skyfire {
	...
	inline SOCKET sf_tcp_server::raw_socket() {
		return listen_sock__;
	}

	void sf_tcp_server::handle_sock_error__(sf_per_handle_data_t* p_handle_data, sf_per_io_operation_data_t* p_io_data, int index)
	{
	...
	void sf_tcp_server::write_handle__(DWORD bytes_transferred, sf_per_handle_data_t* p_handle_data, sf_per_io_operation_data_t* p_io_data, int index)
	{
		auto invalid = false;
		{
			...
			if (p_io_data->data_trans_count != p_io_data->buffer.size()) {
				sf_debug("not send complete", p_io_data->data_trans_count, p_io_data->buffer.size());
	...
	inline void sf_tcp_server::receive_handle__(DWORD bytes_transferred, sf_per_handle_data_t* p_handle_data, sf_per_io_operation_data_t* p_io_data, int index)
	{
		auto invalid = false;
		do {
			// 接收
	...
	inline void sf_tcp_server::server_work_thread__(int index) {
        ...
        while (true) {
	        const auto result = GetQueuedCompletionStatus(completion_port, &bytes_transferred, reinterpret_cast<PULONG_PTR>(&p_handle_data),
            ...
            if (p_io_data->is_send) {
				...
				receive_handle__(bytes_transferred, p_handle_data, p_io_data, index);
            }
        }
    }
	...
	bool sf_tcp_server::add_sock__(SOCKET accept_socket)
	{
		...
		for (auto i = 1; i < thread_count__; ++i)
		{
			auto sz = 0;
			{
				std::shared_lock<std::shared_mutex> lck(*iocp_context__[i].mu_socks);
		...
		{
			std::unique_lock<std::shared_mutex> lck(*iocp_context__[index].mu_socks);
    ...
    inline bool sf_tcp_server::listen(const std::string &ip, unsigned short port) {
		...
		// 启动线程accept
		thread_vec__.emplace_back(std::thread(&sf_tcp_server::accept_thread__, this));
        return true;
    }
