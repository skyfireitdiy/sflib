...
namespace skyfire {
    ...
    struct sf_http_server_config final
    {
        ...
        int max_cache_file_size = default_cache_max_file_size; // 最大cache 文件大小，超过此大小的文件不缓存
        bool debug = false;     // debug 模式不会使用缓存
    };

    SF_JSONIFY(sf_http_server_config, host, port, tmp_file_path, session_timeout, max_cache_count,
            max_cache_file_size, debug)
}
