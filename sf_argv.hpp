...
namespace skyfire {
    inline sf_argparser::sf_argparser(const std::string &help, const std::pair<std::string, std::string> &prefix)
            : help_(help),
              prefix_(prefix) {

    }

    inline std::shared_ptr<sf_argparser>
    sf_argparser::make_parser(const std::string &help, const std::pair<std::string, std::string> &prefix) {
        return  std::shared_ptr<sf_argparser>(new sf_argparser(help, prefix));
    }

    inline void sf_argparser::add_sub_parser(const std::string &name,
    ...
    inline sf_json sf_argparser::parse_argv__(const std::vector<std::string> &argv) const {
        sf_json ret;
        ...
        for (const std::string &p:argv) {
            if (!last_opt) {
                ...
                auto d = sf_json();
                last_opt = std::shared_ptr<sf_argv_opt_t>(new sf_argv_opt_t(position_arg_[pos]));
                ++pos;
    ...
    inline sf_json sf_argparser::parse_argv(int argc, const char **argv, bool skip0) const
    {
        ...
        for(int i=0;i<data.size();++i)
        {
            ...
            if(!find_flag)
            {
                return parser->parse_argv__({data.begin()+i, data.end()});
            }
        }
        return sf_json();
    }
    ...
    bool
    sf_argparser::add_argument(std::string short_name,
