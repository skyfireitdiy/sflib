...
namespace skyfire
{
std::unordered_map<int, std::vector<color>> log_color_map = {
...
int logger::add_level_stream(const int     level,
                             std::ostream* os,
...
int logger::add_level_file(const int          level,
...
void logger::remove_filter(const int key)
{
    std::unique_lock<std::recursive_mutex> lock(func_set_mutex__);
...
bool logger::check_key_can_use__(const int key)
{
    std::unique_lock<std::recursive_mutex> lock(func_set_mutex__);
...
int logger::make_random_logger_id__()
{
...
std::string logger::format(std::string          format_str,
                           const logger_info_t& log_info, bool colored)
{
