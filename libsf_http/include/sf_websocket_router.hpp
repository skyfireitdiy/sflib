...
namespace skyfire {
    ...
    sf_websocket_router::sf_websocket_router(const std::string &url,
                                             std::function<void(const websocket_param_t &)> callback) :
            url__(url), callback__(callback) {

    }

    sf_websocket_router::sf_websocket_router(const std::string &url, void (*callback)(const websocket_param_t &)) :
            url__(url), callback__(std::function(callback)) {

    }

    bool sf_websocket_router::run_route(const websocket_param_t &context) {
