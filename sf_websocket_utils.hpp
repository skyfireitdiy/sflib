...
namespace skyfire {
    ...
    byte_array make_server_websocket_data_pkg(const T &data) {
        ...
        if(data.size() < 126){
            ...
            *reinterpret_cast<unsigned short*>(header.extend_len) = htons(static_cast<unsigned short>(data.size()));
            ...
            header.fin_rsv_oc = 0b10000000;
            header.fin_rsv_oc |= type;
            *reinterpret_cast<unsigned short*>(header.extend_len) = static_cast<unsigned short>(sf_hton64(static_cast<unsigned short>(data.size())));
            ret += to_byte_array(header);
        }
        if constexpr (std::is_same_v<T,std::string>) {
    ...
    inline unsigned long long sf_get_size(const sf_websocket_client_data_1_header_t &header) {
    ...
    inline unsigned long long sf_get_size(const sf_websocket_client_data_2_header_t &header) {
    ...
    inline unsigned long long sf_get_size(const sf_websocket_client_data_3_header_t &header)
    {
