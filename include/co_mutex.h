...
template <typename T>
bool co_timed_mutex::try_lock_for(const T& t)
{
    return try_lock_until(std::chrono::system_clock::now() + t);
}

template <typename T>
bool co_timed_mutex::try_lock_until(const T& t)
{
...
template <typename T>
bool co_recursive_timed_mutex::try_lock_until(const T& t)
{
    while (std::chrono::system_clock::now() < t)
    {
        if (mu__.try_lock())
...
template <typename T>
bool co_shared_timed_mutex::try_lock_for(const T& t)
{
    return try_lock_until(std::chrono::system_clock::now() + t);
}

template <typename T>
bool co_shared_timed_mutex::try_lock_until(const T& t)
{
    while (std::chrono::system_clock::now() < t)
    {
        if (mu__.try_lock())
