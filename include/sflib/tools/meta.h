...
{
    static constexpr bool value = check_param_reference<decltype(std::function(std::declval<T>()))>::value;
...
{
    static constexpr bool value = check_param_pointer<decltype(std::function(std::declval<T>()))>::value;
...
{
    using return_type = typename function_type_helper<decltype(std::function(std::declval<T>()))>::return_type;
    using param_type = typename function_type_helper<decltype(std::function(std::declval<T>()))>::param_type;
};
...
auto make_placeholders()
{
    return typename decltype(__placeholders_type(std::placeholders::_1))::template type<N> {};
}
template <typename _Type, typename Tuple>
_Type *make_obj_from_tuple(Tuple &&t)
{
